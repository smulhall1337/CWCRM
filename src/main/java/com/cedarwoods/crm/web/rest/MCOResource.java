...
public class MCOResource {
    ...
    /**
     * SEARCH  /_search/mcos?query=:query : search for the mCO corresponding
     * to the query.
     *
     * @param query the query of the mCO search
     * @return the result of the search
     */
    @GetMapping("/_search/mcos")
    @Timed
    public List<MCODTO> searchMCOS(@RequestParam String query) {
        log.debug("REST request to search MCOS for query {}", query);
        return mCOService.search(query);
    }
