...
public class SupportCoordinatorResource {
    ...
    /**
     * SEARCH  /_search/support-coordinators?query=:query : search for the supportCoordinator corresponding
     * to the query.
     *
     * @param query the query of the supportCoordinator search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/support-coordinators")
    @Timed
    public ResponseEntity<List<SupportCoordinatorDTO>> searchSupportCoordinators(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of SupportCoordinators for query {}", query);
        Page<SupportCoordinatorDTO> page = supportCoordinatorService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/support-coordinators");
