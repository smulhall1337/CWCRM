...
public class ContactSubStatusResource {
    ...
    /**
     * SEARCH  /_search/contact-sub-statuses?query=:query : search for the contactSubStatus corresponding
     * to the query.
     *
     * @param query the query of the contactSubStatus search
     * @return the result of the search
     */
    @GetMapping("/_search/contact-sub-statuses")
    @Timed
    public List<ContactSubStatusDTO> searchContactSubStatuses(@RequestParam String query) {
        log.debug("REST request to search ContactSubStatuses for query {}", query);
        return contactSubStatusService.search(query);
    }
