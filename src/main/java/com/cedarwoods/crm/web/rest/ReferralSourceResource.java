...
public class ReferralSourceResource {
    ...
    /**
     * SEARCH  /_search/referral-sources?query=:query : search for the referralSource corresponding
     * to the query.
     *
     * @param query the query of the referralSource search
     * @return the result of the search
     */
    @GetMapping("/_search/referral-sources")
    @Timed
    public List<ReferralSourceDTO> searchReferralSources(@RequestParam String query) {
        log.debug("REST request to search ReferralSources for query {}", query);
        return referralSourceService.search(query);
    }
