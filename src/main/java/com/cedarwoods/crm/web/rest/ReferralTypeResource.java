...
public class ReferralTypeResource {
    ...
    /**
     * SEARCH  /_search/referral-types?query=:query : search for the referralType corresponding
     * to the query.
     *
     * @param query the query of the referralType search
     * @return the result of the search
     */
    @GetMapping("/_search/referral-types")
    @Timed
    public List<ReferralTypeDTO> searchReferralTypes(@RequestParam String query) {
        log.debug("REST request to search ReferralTypes for query {}", query);
        return referralTypeService.search(query);
    }
