...
public class EnrollmentAgencyResource {
    ...
    /**
     * SEARCH  /_search/enrollment-agencies?query=:query : search for the enrollmentAgency corresponding
     * to the query.
     *
     * @param query the query of the enrollmentAgency search
     * @return the result of the search
     */
    @GetMapping("/_search/enrollment-agencies")
    @Timed
    public List<EnrollmentAgencyDTO> searchEnrollmentAgencies(@RequestParam String query) {
        log.debug("REST request to search EnrollmentAgencies for query {}", query);
        return enrollmentAgencyService.search(query);
    }
