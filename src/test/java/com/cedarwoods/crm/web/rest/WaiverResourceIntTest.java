...
public class WaiverResourceIntTest {
     ...
     *
     * @see com.cedarwoods.crm.repository.search.WaiverSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createWaiver() throws Exception {
        ...
        // Validate the Waiver in Elasticsearch
        verify(mockWaiverSearchRepository, times(1)).save(testWaiver);
    }
    ...
    public void createWaiverWithExistingId() throws Exception {
        ...
        // Validate the Waiver in Elasticsearch
        verify(mockWaiverSearchRepository, times(0)).save(waiver);
    }
    ...
    public void deleteWaiver() throws Exception {
        ...
        // Validate the Waiver in Elasticsearch
        verify(mockWaiverSearchRepository, times(1)).deleteById(waiver.getId());
    }

    @Test
    @Transactional
    public void searchWaiver() throws Exception {
        // Initialize the database
        waiverRepository.saveAndFlush(waiver);
        when(mockWaiverSearchRepository.search(queryStringQuery("id:" + waiver.getId())))
            .thenReturn(Collections.singletonList(waiver));
        // Search the waiver
        restWaiverMockMvc.perform(get("/api/_search/waivers?query=id:" + waiver.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(waiver.getId().intValue())))
