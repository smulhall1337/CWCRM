...
import com.cedarwoods.crm.domain.Department;
...
public class SupportCoordinatorResourceIntTest {
     ...
     *
     * @see com.cedarwoods.crm.repository.search.SupportCoordinatorSearchRepositoryMockConfiguration
     */
    @Autowired
    private SupportCoordinatorSearchRepository mockSupportCoordinatorSearchRepository;
    ...
    public void createSupportCoordinator() throws Exception {
        ...
        // Validate the SupportCoordinator in Elasticsearch
        verify(mockSupportCoordinatorSearchRepository, times(1)).save(testSupportCoordinator);
    }
    ...
    public void createSupportCoordinatorWithExistingId() throws Exception {
        ...
        // Validate the SupportCoordinator in Elasticsearch
        verify(mockSupportCoordinatorSearchRepository, times(0)).save(supportCoordinator);
    }
    ...
    public void deleteSupportCoordinator() throws Exception {
        ...
        // Validate the SupportCoordinator in Elasticsearch
        verify(mockSupportCoordinatorSearchRepository, times(1)).deleteById(supportCoordinator.getId());
    }

    @Test
    @Transactional
    public void searchSupportCoordinator() throws Exception {
        // Initialize the database
        supportCoordinatorRepository.saveAndFlush(supportCoordinator);
        when(mockSupportCoordinatorSearchRepository.search(queryStringQuery("id:" + supportCoordinator.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(supportCoordinator), PageRequest.of(0, 1), 1));
        // Search the supportCoordinator
        restSupportCoordinatorMockMvc.perform(get("/api/_search/support-coordinators?query=id:" + supportCoordinator.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(supportCoordinator.getId().intValue())))
