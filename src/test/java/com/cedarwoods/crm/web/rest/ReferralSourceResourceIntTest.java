...
public class ReferralSourceResourceIntTest {
     ...
     *
     * @see com.cedarwoods.crm.repository.search.ReferralSourceSearchRepositoryMockConfiguration
     */
    @Autowired
    private ReferralSourceSearchRepository mockReferralSourceSearchRepository;
    ...
    public void createReferralSource() throws Exception {
        ...
        // Validate the ReferralSource in Elasticsearch
        verify(mockReferralSourceSearchRepository, times(1)).save(testReferralSource);
    }
    ...
    public void createReferralSourceWithExistingId() throws Exception {
        ...
        // Validate the ReferralSource in Elasticsearch
        verify(mockReferralSourceSearchRepository, times(0)).save(referralSource);
    }
    ...
    public void deleteReferralSource() throws Exception {
        ...
        // Validate the ReferralSource in Elasticsearch
        verify(mockReferralSourceSearchRepository, times(1)).deleteById(referralSource.getId());
    }

    @Test
    @Transactional
    public void searchReferralSource() throws Exception {
        // Initialize the database
        referralSourceRepository.saveAndFlush(referralSource);
        when(mockReferralSourceSearchRepository.search(queryStringQuery("id:" + referralSource.getId())))
            .thenReturn(Collections.singletonList(referralSource));
        // Search the referralSource
        restReferralSourceMockMvc.perform(get("/api/_search/referral-sources?query=id:" + referralSource.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(referralSource.getId().intValue())))
