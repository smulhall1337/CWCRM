...
public class EmployeeSubTypeResourceIntTest {
     ...
     *
     * @see com.cedarwoods.crm.repository.search.EmployeeSubTypeSearchRepositoryMockConfiguration
     */
    @Autowired
    private EmployeeSubTypeSearchRepository mockEmployeeSubTypeSearchRepository;
    ...
    public void createEmployeeSubType() throws Exception {
        ...
        // Validate the EmployeeSubType in Elasticsearch
        verify(mockEmployeeSubTypeSearchRepository, times(1)).save(testEmployeeSubType);
    }
    ...
    public void createEmployeeSubTypeWithExistingId() throws Exception {
        ...
        // Validate the EmployeeSubType in Elasticsearch
        verify(mockEmployeeSubTypeSearchRepository, times(0)).save(employeeSubType);
    }
    ...
    public void deleteEmployeeSubType() throws Exception {
        ...
        // Validate the EmployeeSubType in Elasticsearch
        verify(mockEmployeeSubTypeSearchRepository, times(1)).deleteById(employeeSubType.getId());
    }

    @Test
    @Transactional
    public void searchEmployeeSubType() throws Exception {
        // Initialize the database
        employeeSubTypeRepository.saveAndFlush(employeeSubType);
        when(mockEmployeeSubTypeSearchRepository.search(queryStringQuery("id:" + employeeSubType.getId())))
            .thenReturn(Collections.singletonList(employeeSubType));
        // Search the employeeSubType
        restEmployeeSubTypeMockMvc.perform(get("/api/_search/employee-sub-types?query=id:" + employeeSubType.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(employeeSubType.getId().intValue())))
