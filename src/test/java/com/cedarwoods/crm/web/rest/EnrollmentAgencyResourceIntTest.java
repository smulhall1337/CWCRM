...
public class EnrollmentAgencyResourceIntTest {
     ...
     *
     * @see com.cedarwoods.crm.repository.search.EnrollmentAgencySearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createEnrollmentAgency() throws Exception {
        ...
        // Validate the EnrollmentAgency in Elasticsearch
        verify(mockEnrollmentAgencySearchRepository, times(1)).save(testEnrollmentAgency);
    }
    ...
    public void createEnrollmentAgencyWithExistingId() throws Exception {
        ...
        // Validate the EnrollmentAgency in Elasticsearch
        verify(mockEnrollmentAgencySearchRepository, times(0)).save(enrollmentAgency);
    }
    ...
    public void deleteEnrollmentAgency() throws Exception {
        ...
        // Validate the EnrollmentAgency in Elasticsearch
        verify(mockEnrollmentAgencySearchRepository, times(1)).deleteById(enrollmentAgency.getId());
    }

    @Test
    @Transactional
    public void searchEnrollmentAgency() throws Exception {
        // Initialize the database
        enrollmentAgencyRepository.saveAndFlush(enrollmentAgency);
        when(mockEnrollmentAgencySearchRepository.search(queryStringQuery("id:" + enrollmentAgency.getId())))
            .thenReturn(Collections.singletonList(enrollmentAgency));
        // Search the enrollmentAgency
        restEnrollmentAgencyMockMvc.perform(get("/api/_search/enrollment-agencies?query=id:" + enrollmentAgency.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(enrollmentAgency.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
            .andExpect(jsonPath("$.[*].office").value(hasItem(DEFAULT_OFFICE)))
            .andExpect(jsonPath("$.[*].recordNumber").value(hasItem(DEFAULT_RECORD_NUMBER)))
