...
public class ContactSubStatusResourceIntTest {
     ...
     *
     * @see com.cedarwoods.crm.repository.search.ContactSubStatusSearchRepositoryMockConfiguration
     */
    @Autowired
    private ContactSubStatusSearchRepository mockContactSubStatusSearchRepository;
    ...
    public void createContactSubStatus() throws Exception {
        ...
        // Validate the ContactSubStatus in Elasticsearch
        verify(mockContactSubStatusSearchRepository, times(1)).save(testContactSubStatus);
    }
    ...
    public void createContactSubStatusWithExistingId() throws Exception {
        ...
        // Validate the ContactSubStatus in Elasticsearch
        verify(mockContactSubStatusSearchRepository, times(0)).save(contactSubStatus);
    }
    ...
    public void deleteContactSubStatus() throws Exception {
        ...
        // Validate the ContactSubStatus in Elasticsearch
        verify(mockContactSubStatusSearchRepository, times(1)).deleteById(contactSubStatus.getId());
    }

    @Test
    @Transactional
    public void searchContactSubStatus() throws Exception {
        // Initialize the database
        contactSubStatusRepository.saveAndFlush(contactSubStatus);
        when(mockContactSubStatusSearchRepository.search(queryStringQuery("id:" + contactSubStatus.getId())))
            .thenReturn(Collections.singletonList(contactSubStatus));
        // Search the contactSubStatus
        restContactSubStatusMockMvc.perform(get("/api/_search/contact-sub-statuses?query=id:" + contactSubStatus.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(contactSubStatus.getId().intValue())))
