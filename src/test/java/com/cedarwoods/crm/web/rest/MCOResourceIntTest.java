...
public class MCOResourceIntTest {
     ...
     *
     * @see com.cedarwoods.crm.repository.search.MCOSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createMCO() throws Exception {
        ...
        // Validate the MCO in Elasticsearch
        verify(mockMCOSearchRepository, times(1)).save(testMCO);
    }
    ...
    public void createMCOWithExistingId() throws Exception {
        ...
        // Validate the MCO in Elasticsearch
        verify(mockMCOSearchRepository, times(0)).save(mCO);
    }
    ...
    public void deleteMCO() throws Exception {
        ...
        // Validate the MCO in Elasticsearch
        verify(mockMCOSearchRepository, times(1)).deleteById(mCO.getId());
    }

    @Test
    @Transactional
    public void searchMCO() throws Exception {
        // Initialize the database
        mCORepository.saveAndFlush(mCO);
        when(mockMCOSearchRepository.search(queryStringQuery("id:" + mCO.getId())))
            .thenReturn(Collections.singletonList(mCO));
        // Search the mCO
        restMCOMockMvc.perform(get("/api/_search/mcos?query=id:" + mCO.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(mCO.getId().intValue())))
