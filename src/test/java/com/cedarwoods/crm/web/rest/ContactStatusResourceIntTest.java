...
public class ContactStatusResourceIntTest {
     ...
     *
     * @see com.cedarwoods.crm.repository.search.ContactStatusSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createContactStatus() throws Exception {
        ...
        // Validate the ContactStatus in Elasticsearch
        verify(mockContactStatusSearchRepository, times(1)).save(testContactStatus);
    }
    ...
    public void createContactStatusWithExistingId() throws Exception {
        ...
        // Validate the ContactStatus in Elasticsearch
        verify(mockContactStatusSearchRepository, times(0)).save(contactStatus);
    }
    ...
    public void deleteContactStatus() throws Exception {
        ...
        // Validate the ContactStatus in Elasticsearch
        verify(mockContactStatusSearchRepository, times(1)).deleteById(contactStatus.getId());
    }

    @Test
    @Transactional
    public void searchContactStatus() throws Exception {
        // Initialize the database
        contactStatusRepository.saveAndFlush(contactStatus);
        when(mockContactStatusSearchRepository.search(queryStringQuery("id:" + contactStatus.getId())))
            .thenReturn(Collections.singletonList(contactStatus));
        // Search the contactStatus
        restContactStatusMockMvc.perform(get("/api/_search/contact-statuses?query=id:" + contactStatus.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(contactStatus.getId().intValue())))
