...
public class ReferralTypeResourceIntTest {
     ...
     *
     * @see com.cedarwoods.crm.repository.search.ReferralTypeSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createReferralType() throws Exception {
        ...
        // Validate the ReferralType in Elasticsearch
        verify(mockReferralTypeSearchRepository, times(1)).save(testReferralType);
    }
    ...
    public void createReferralTypeWithExistingId() throws Exception {
        ...
        // Validate the ReferralType in Elasticsearch
        verify(mockReferralTypeSearchRepository, times(0)).save(referralType);
    }
    ...
    public void deleteReferralType() throws Exception {
        ...
        // Validate the ReferralType in Elasticsearch
        verify(mockReferralTypeSearchRepository, times(1)).deleteById(referralType.getId());
    }

    @Test
    @Transactional
    public void searchReferralType() throws Exception {
        // Initialize the database
        referralTypeRepository.saveAndFlush(referralType);
        when(mockReferralTypeSearchRepository.search(queryStringQuery("id:" + referralType.getId())))
            .thenReturn(Collections.singletonList(referralType));
        // Search the referralType
        restReferralTypeMockMvc.perform(get("/api/_search/referral-types?query=id:" + referralType.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(referralType.getId().intValue())))
