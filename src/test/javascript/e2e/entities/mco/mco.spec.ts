...
describe('MCO e2e test', () => {
    ...
    it('should load MCOS', async () => {
        ...
        expect(await mCOComponentsPage.getTitle()).to.eq('cwcrmApp.mCO.home.title');
    });

    it('should load create MCO page', async () => {
        await mCOComponentsPage.clickOnCreateButton();
        mCOUpdatePage = new MCOUpdatePage();
        expect(await mCOUpdatePage.getPageTitle()).to.eq('cwcrmApp.mCO.home.createOrEditLabel');
    ...
    it('should delete last MCO', async () => {
        ...
        expect(await mCODeleteDialog.getDialogTitle()).to.eq('cwcrmApp.mCO.delete.question');
